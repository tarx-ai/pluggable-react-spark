...
const RightSidebar = ({ className, visible, onToggle }: RightSidebarProps) => {
    ...
    return (
        <>
            <div
                className={twMerge(
                    `absolute top-0 right-0 bottom-0 flex flex-col w-[22.5rem] pt-[8rem] pb-24 bg-n-1 rounded-r-[1.25rem] border-l border-n-3 shadow-[inset_0_1.5rem_3.75rem_rgba(0,0,0,0.1)] transition-all duration-300 2xl:w-80 lg:rounded-[1.25rem] lg:z-20 lg:border-l-0 lg:shadow-2xl md:fixed md:w-[calc(100%-4rem)] md:border-l md:rounded-none dark:bg-n-6 dark:border-n-5 ${
                        visible 
                            ? "lg:visible lg:opacity-100 lg:translate-x-0" 
                            : "lg:invisible lg:opacity-0 lg:translate-x-full"
                    } ${className}`
                )}
            >
                <div className="absolute top-0 left-0 right-0 flex justify-between items-center h-18 px-9 border-b border-n-3 lg:pr-18 md:pr-16 dark:border-n-5">
                    {onToggle && (
                        <button
